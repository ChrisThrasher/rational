...
std::ostream& operator<<(std::ostream& out, const Rational<IntType>& value)
{
    return out << "(" << +value.m_num << " / " << +value.m_den << ")";
}

template <typename IntType>
struct std::numeric_limits<Rational<IntType>> : std::numeric_limits<IntType> {
    static constexpr bool is_integer { false };

    static constexpr Rational<IntType> min() noexcept
    {
        return { std::numeric_limits<IntType>::min() };
    }

    static constexpr Rational<IntType> lowest() noexcept
    {
        return { std::numeric_limits<IntType>::lowest() };
    }

    static constexpr Rational<IntType> max() noexcept
    {
